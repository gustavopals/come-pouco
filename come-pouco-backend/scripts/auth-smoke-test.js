...
const deriveKey = () =>
  crypto
    .createHash('sha256')
    .update(process.env.TWOFA_ENCRYPTION_KEY || 'dev-twofa-encryption-key-change-me')
    .digest();
...
(async () => {
  ...
  const useExternalServer =
    String(process.env.AUTH_SMOKE_USE_EXTERNAL || 'false').toLowerCase() === 'true';
  ...
  checks.push([
    'wrong_password',
    res.status === 401 && res.data.errorCode === 'AUTH_INVALID_CREDENTIALS'
  ]);

  await db.query(
    `UPDATE users SET two_factor_enabled = true, two_factor_secret = $1 WHERE username = $2`,
    [encryptSecret(secret), username]
  );
  ...
  const wrongVerify = await request(
    '/auth/login/2fa',
    { tempToken: challenge, code: '000000' },
    null
  );
  checks.push([
    'verify_wrong_code',
    wrongVerify.status === 400 && wrongVerify.data.errorCode === 'AUTH_INVALID_2FA_CODE'
  ]);
  ...
  const disableInvalid = await request(
    '/auth/2fa/disable',
  ...
  checks.push([
    ...
    disableInvalid.status === 400 && disableInvalid.data.errorCode === 'AUTH_INVALID_2FA_CODE'
  ]);

  if (!useExternalServer) {
    throw new Error(
      'AUTH_SMOKE_USE_EXTERNAL=true is required in this environment (process spawn is restricted).'
    );
  }

  await db.query(
    `UPDATE users SET two_factor_enabled = false, two_factor_secret = null WHERE username = $1`,
    [username]
  );
  await db.end();
